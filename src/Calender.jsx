...
let Calender = () => {
  ...
  return (
    <>
      <div
        className={`calender ${
          isActivePatient || isApproval ? "col-5" : "col-10"
        }`}
      >
        <nav class="navbar navbar-expand-lg navbar-dark bg-dark">
          <div class="container-fluid d-flex justify-content-between">
            ...
            <form class="d-flex bd-highlight col-10">
              <Link class="nav-link white col-2" to="/addpatient">
                Add Patient
              </Link>
              <li
                class={`nav-link col-3 ${
                  appointmentRequest.length > 0 ? "red" : "white"
                }`}
                aria-current="page"
                onClick={() => {
                  setIsApproval(true);
      ...
      <div
        className={`col-4 ${
          isActivePatient || isApproval ? "activeDiv" : "hide"
        }`}
      >
        {isApproval === true ? (
          <>
            {appointmentRequest.map((e) => {
              return (
                <div className="singlePatient m-4 p-4">
                  <div>
            ...
            <div className="content p-5">
              {activePatient !== null ? (
                <>
                  ...
                  <div>{`Date of Appointment :  ${activePatient.start
                    .toISOString()
                    .substring(0, 10)}`}</div>
      ...
      <button
        ...
        class={`btn btn-danger col-1 ${
          isActivePatient || isApproval ? "backBtn" : "hide"
        }`}
        onClick={() => {
          setIsActivePatient(false);
          setIsApproval(false);
